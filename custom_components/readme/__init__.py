...
def create_initial_files(hass: HomeAssistant):
    ...
    if not os.path.exists(hass.config.path("addons/readme")):
        os.mkdir(hass.config.path("addons/readme"))

    if not os.path.exists(hass.config.path("addons/readme/README.j2")):

        copyfile(
            hass.config.path("custom_components/readme/default.j2"),
            hass.config.path("addons/readme/README.j2"),
        )
...
async def add_services(hass: HomeAssistant):
    ...
    async def service_generate(_call):
        ...
        content = await read_file(hass, "addons/readme/README.j2")
